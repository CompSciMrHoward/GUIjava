 ...
 * Chapter 12- Pizza Place project
 * PizzaPlace will display a pizza ordering screen.
 * When the "Place Order" button is clicked a message pop-up
 * will display a summary of the order.  When the 
 * "Reset Values" button is clicked, all controls will 
...
public class PizzaPlace implements ActionListener
{
	...
	JComboBox toppingList;		// JList for holding the selected toppings
	...
	public PizzaPlace()
	{
		...
		toppingList = new JComboBox(toppings);
		...
		//JScrollPane topScroll = new JScrollPane(toppingList, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		JScrollPane commentScroll = new JScrollPane(commentArea, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getSource() == orderButton)
		{
			...
			pizzaInfo = pizzaInfo + "  " + (String)crustCombo.getSelectedItem() + "\n";
			
			// Add topping information based on the selected values in the list box
			pizzaInfo = pizzaInfo + "TOPPINGS:\n";
			Object[] toppings = toppingList.getSelectedObjects(); 
		...
		else if (e.getSource() == resetButton)
		{
			...
			toppingList.setSelectedIndex(0);
